...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($senha !== $confirma) {
        ...
        if ($res['ok']) {
            ...
            login(trim($_POST['email']), $senha);
            if ($res['role'] === 'admin') {
                $aviso = 'admin_first';
            }
            header('Location: ' . BASE_PATH . '/index.php' . ($aviso ? '?aviso=' . $aviso : ''));
            exit;
        }
        $erro = $res['erro'];
    }
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    :root{--bg:#05080c;--card:#0b1520;--bd:#162130;--amber:#f59e0b;--sky:#38bdf8;--red:#f87171;--grn:#4ade80;--txt:#b8cce0;--mut:#364f68}
    ...
    .info{background:#38bdf818;border:1px solid #38bdf833;border-radius:6px;padding:10px 13px;font-size:12px;color:var(--sky);margin-bottom:16px}
...
<div class="card">
  ...
  <h1>Criar conta</h1>
  <p class="sub">Acesso ao supervisório</p>
  <div class="info">
    O primeiro usuário cadastrado recebe role <strong>admin</strong> automaticamente.
    Os demais recebem <strong>visitor</strong> (somente leitura).
  </div>
  <?php if ($erro): ?>
  ...
  <form method="POST" novalidate>
    ...
    <label>Senha <span style="color:var(--mut);font-size:9px">(mín. 6 caracteres)</span></label>
  ...
  <div class="footer">Já tem conta? <a href="<?= BASE_PATH ?>/login.php">Entrar</a></div>
</div>
</body>
</html>
